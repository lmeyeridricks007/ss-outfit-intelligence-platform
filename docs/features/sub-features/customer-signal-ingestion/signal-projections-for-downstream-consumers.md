# Sub-feature capability: Signal Projections For Downstream Consumers
...
Project governed signal envelopes into session, profile, and channel-specific read models so recommendation systems do not need to replay raw signal history on every request.
...
This capability sits under `Customer signal ingestion` and owns one clear responsibility: project governed signal envelopes into session, profile, and channel-specific read models so recommendation systems do not need to replay raw signal history on every request.
...
- Prevents downstream consumers from reinterpreting `Signal Projections For Downstream Consumers` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A signal becomes eligible, stale, expired, or revoked.
- A downstream consumer requests a projection keyed by customer, session, or channel.
- Projection schemas or feature consumers evolve.

## 5. Inputs

- eligible and freshness-classified signals
- projection configuration by consumer
- identity and session linkage
- returns or cancellation adjustments
- projection invalidation markers

## 6. Outputs

- session-intent projections
- durable customer-history projections
- channel-specific signal summaries
- projection freshness metadata

## 7. Workflow / Lifecycle

1. Select the eligible signals relevant to the target projection.
2. Aggregate and summarize them into a consumer-friendly read model.
3. Persist the projection with freshness, lineage, and confidence metadata.
4. Invalidate or recompute projections when upstream signal state changes.
5. Serve the projections to profile, ranking, analytics, and channel consumers.
...
- SignalProjection
- SessionIntentProjection
- CustomerHistoryProjection
- ProjectionFreshnessRecord
- ProjectionInvalidationRecord
...
- GET /signals/projections/{projectionKey}
- POST /signals/projections/rebuild
...
- signal.projection.updated
- signal.projection.invalidated

## 13. Events Consumed

- signal.identity-gating.updated
- commerce.order.adjusted
...
1. A request or upstream change triggers `Signal Projections For Downstream Consumers`.
...
```json
{
  "subFeature": "signal-projections-for-downstream-consumers",
  "feature": "customer-signal-ingestion",
  "input": "eligible and freshness-classified signals",
  "output": "session-intent projections",
...
- Backend services: add or extend a `signal-projections-for-downstream-consumers` service boundary under the `customer-signal-ingestion` domain.
