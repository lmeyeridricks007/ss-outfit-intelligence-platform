...
Project recommendation telemetry into dashboards and reports for funnel health, experiment results, governance effects, and operational quality.

## 2. Core Concept

Reporting Projections and Dashboards is the capability slice of **Analytics and Experimentation** that turns the parent feature intent into a bounded implementation contract with explicit lifecycle, contracts, and operational behavior.

## 3. User Problems Solved

- Downstream teams need reporting projections and dashboards behavior that is explicitly defined instead of inferred from the parent feature doc.
- Recommendation-serving paths need implementation-ready contracts for reporting projections and dashboards so engineering does not guess at lifecycle or data semantics.
...
- Telemetry ingestion completes for a reporting interval
- An operator or analyst requests a report
- A dashboard data quality alert fires

## 5. Inputs

- Telemetry and attribution data
- Report definitions
...
- Dashboard views
- Quality annotations on reports

## 7. Workflow / Lifecycle

1. Aggregate telemetry and attributed outcomes into curated reporting models.
2. Render dashboards and scheduled reports using consistent definitions.
3. Attach freshness and data quality annotations where reporting is incomplete.

## 8. Business Rules

- Dashboard metrics must align with published event and attribution definitions.
- Reports need visible freshness and completeness status.
- Operational dashboards must distinguish business changes from telemetry degradation.

## 9. Configuration Model

- Reporting Projections and Dashboards policy version, rollout state, and owner metadata.
...
- `reporting_projections_and_dashboards` primary record storing the current reporting projections and dashboards state.
- `reporting_projections_and_dashboards_history` append-only history for lifecycle and trace reconstruction.
- `reporting_projections_and_dashboards_projection` read model optimized for downstream serving or operator lookup.
...
- GET /internal/analytics/reports/{reportId}
- POST /internal/analytics/reports/refresh

## 12. Events Produced

- RecommendationReportRefreshed
- RecommendationReportDataQualityDegraded

## 13. Events Consumed

- RecommendationOutcomeAttributed
- TelemetryQualityEvaluated
...
- Metrics: request count, success rate, degraded rate, and freshness for reporting projections and dashboards.
...
1. API flow: call `GET /internal/analytics/reports/{reportId}` with `Telemetry and attribution data` and return `Reporting projections` plus traceable metadata.
2. Event flow: consume `RecommendationOutcomeAttributed` and emit `RecommendationReportRefreshed` after business rules and lifecycle checks pass.
3. Operator flow: use `Recommendation analytics dashboard` to inspect or change reporting projections and dashboards behavior, then verify the outcome in the linked trace or health view.

## 23. Implementation Notes

- Backend services involved: `analytics-and-experimentation-service`, `analytics-and-experimentation-api`, and `reporting-projections-and-dashboards-worker` for async processing.
- Database tables or collections required: `reporting_projections_and_dashboards`, `reporting_projections_and_dashboards_history`, and `reporting_projections_and_dashboards_projection`.
- Jobs or workers required: `reporting-projections-and-dashboards-worker`, nightly reconciliation, and replay tooling for operational recovery.
- External APIs or systems required: Analytics warehouse, BI tooling, Operator admin UI.
- Frontend components required: Funnel chart, Data freshness ribbon.
...
- What reporting latency is acceptable for daily optimization versus executive summaries?
- Which dashboards belong in-product versus external BI tools?
