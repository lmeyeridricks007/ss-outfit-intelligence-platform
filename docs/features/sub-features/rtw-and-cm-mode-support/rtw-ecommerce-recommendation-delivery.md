# Sub-feature capability: RTW Ecommerce Recommendation Delivery
...
Apply RTW-first ecommerce recommendation behavior with immediate inventory-aware delivery, duplicate handling, and customer-facing readiness expectations suitable for Phase 1 web flows.
...
This capability sits under `RTW and CM mode support` and owns one clear responsibility: apply rtw-first ecommerce recommendation behavior with immediate inventory-aware delivery, duplicate handling, and customer-facing readiness expectations suitable for phase 1 web flows.
...
- Prevents downstream consumers from reinterpreting `RTW Ecommerce Recommendation Delivery` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A PDP or cart request explicitly uses RTW mode.
- An RTW recommendation set needs surface-safe validation.
- Inventory or assortment changes affect RTW customer-facing eligibility.

## 5. Inputs

- RTW request context
- catalog and inventory projections
- delivery response and surface policy
- duplicate-suppression rules
- Phase 1 latency and degraded-state expectations

## 6. Outputs

- RTW-safe recommendation payload
- surface-ready candidate selection
- duplicate-suppression markers
- mode-aware telemetry annotations

## 7. Workflow / Lifecycle

1. Validate that the request and product context are RTW-compatible.
2. Use RTW readiness and inventory rules to constrain the candidate pool and final delivery.
3. Apply duplicate suppression and surface-safe degraded handling for ecommerce.
4. Return the RTW recommendation response with mode annotations preserved.
5. Publish mode-sliced telemetry for analytics and rollout tracking.
...
- RTWDeliveryRecord
- RTWEligibilityVerdict
- RTWDuplicateSuppression
- ModeTelemetryAnnotation
- SurfaceSafetyMarker
...
1. A request or upstream change triggers `RTW Ecommerce Recommendation Delivery`.
...
```json
{
  "subFeature": "rtw-ecommerce-recommendation-delivery",
  "feature": "rtw-and-cm-mode-support",
  "input": "RTW request context",
  "output": "RTW-safe recommendation payload",
...
- Backend services: add or extend a `rtw-ecommerce-recommendation-delivery` service boundary under the `rtw-and-cm-mode-support` domain.
