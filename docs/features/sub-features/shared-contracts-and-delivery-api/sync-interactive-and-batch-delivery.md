# Sub-feature capability: Sync Interactive And Batch Delivery
...
Support both low-latency interactive delivery and batch-oriented generation workflows while preserving the same recommendation semantics, identifiers, and freshness expectations across channels.
...
This capability sits under `Shared contracts and delivery API` and owns one clear responsibility: support both low-latency interactive delivery and batch-oriented generation workflows while preserving the same recommendation semantics, identifiers, and freshness expectations across channels.
...
- Prevents downstream consumers from reinterpreting `Sync Interactive And Batch Delivery` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A PDP or cart module requests an interactive response.
- Email or campaign tooling schedules a batch generation job.
- Support or operator tooling requests a preview or regeneration pass.

## 5. Inputs

- normalized request contexts
- delivery mode (`interactive`, `batch`, `preview`, or `regenerate`)
- freshness policies and regeneration limits
- consumer-specific timeout and retry settings
- batch job metadata such as schedule or audience scope

## 6. Outputs

- interactive response payloads with latency-safe semantics
- batch job records and status snapshots
- generated recommendation packages or regeneration requests
- stable recommendation-set references shared across channels

## 7. Workflow / Lifecycle

1. Classify the request as interactive, preview, batch, or regeneration.
2. Route the request through the correct orchestration path while preserving canonical delivery semantics.
3. Apply freshness, retry, and timeout policy for the selected path.
4. Return the interactive response or create trackable batch-generation job state.
5. Emit delivery and job lifecycle events for monitoring and analytics.
...
- GET /recommendations/jobs/{jobId}
...
- support.regeneration.requested
...
1. A request or upstream change triggers `Sync Interactive And Batch Delivery`.
...
```json
{
  "subFeature": "sync-interactive-and-batch-delivery",
  "feature": "shared-contracts-and-delivery-api",
  "input": "normalized request contexts",
  "output": "interactive response payloads with latency-safe semantics",
...
- Backend services: add or extend a `sync-interactive-and-batch-delivery` service boundary under the `shared-contracts-and-delivery-api` domain.
