# Sub-feature capability: Outcome Attribution And Confidence
...
Join exposures to downstream commercial outcomes with explicit confidence and timing rules so reporting reflects honest attribution instead of overclaiming recommendation impact.
...
This capability sits under `Analytics and experimentation` and owns one clear responsibility: join exposures to downstream commercial outcomes with explicit confidence and timing rules so reporting reflects honest attribution instead of overclaiming recommendation impact.
...
- Prevents downstream consumers from reinterpreting `Outcome Attribution And Confidence` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A recommendation impression or interaction precedes a downstream commerce event.
- Late-arriving orders, purchases, or cancellations require attribution updates.
- Analysts or product stakeholders request attributed performance reporting.

## 5. Inputs

- recommendation events
- commerce and order outcomes
- attribution window policy
- identity and session linkage
- returns and cancellation adjustments

## 6. Outputs

- outcome attribution records
- confidence classifications
- adjusted metric inputs for dashboards
- unattributed or low-confidence exception sets

## 7. Workflow / Lifecycle

1. Link outcome events to relevant prior exposures using the configured attribution rules.
2. Calculate confidence based on identity quality, timing, and competing exposures.
3. Persist attributed outcomes and any later adjustments from cancellations or returns.
4. Feed the resulting records into metric snapshots and experiment analysis.
5. Keep low-confidence and unattributed outcomes visible instead of silently discarding them.
...
- GET /analytics/attribution/{recommendationSetId}
...
1. A request or upstream change triggers `Outcome Attribution And Confidence`.
...
```json
{
  "subFeature": "outcome-attribution-and-confidence",
  "feature": "analytics-and-experimentation",
  "input": "recommendation events",
  "output": "outcome attribution records",
...
- Backend services: add or extend a `outcome-attribution-and-confidence` service boundary under the `analytics-and-experimentation` domain.
