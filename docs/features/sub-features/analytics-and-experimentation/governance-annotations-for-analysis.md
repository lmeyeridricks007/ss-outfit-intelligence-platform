...
Attach governance snapshot, campaign, rule, and override context to analytics records so measurement can separate algorithmic movement from operator-driven changes.
...
This capability sits under `Analytics and experimentation` and owns one clear responsibility: attach governance snapshot, campaign, rule, and override context to analytics records so measurement can separate algorithmic movement from operator-driven changes.
...
- Prevents downstream consumers from reinterpreting `Governance Annotations For Analysis` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A recommendation response is delivered under a governance snapshot.
- A campaign or override materially changes recommendation behavior.
- Analysts need to compare behavior before and after governance changes.

## 5. Inputs

- delivery envelope metadata
- governance snapshot and control IDs
- campaign and override annotations
- analytics event records
- trace and experiment linkage

## 6. Outputs

- analytics governance annotations
- enriched event and attribution records
- governance-aware reporting dimensions
- audit-friendly control-change lineage

## 7. Workflow / Lifecycle

1. Receive governance metadata from delivery, ranking, or trace systems.
2. Enrich telemetry and attribution records with the effective governance context.
3. Persist the annotations in reporting-friendly storage.
4. Expose governance filters in dashboards and experiment scorecards.
5. Highlight major behavior shifts caused by governance changes rather than model changes.
...
- ControlChangeLink
- ReportingDimensionRecord
- AnnotationProjection
- GovernanceLiftSegment
...
- POST /analytics/annotations/governance
- GET /analytics/annotations/governance/{recommendationSetId}
...
1. A request or upstream change triggers `Governance Annotations For Analysis`.
...
```json
{
  "subFeature": "governance-annotations-for-analysis",
  "feature": "analytics-and-experimentation",
  "input": "delivery envelope metadata",
  "output": "analytics governance annotations",
...
- Backend services: add or extend a `governance-annotations-for-analysis` service boundary under the `analytics-and-experimentation` domain.
