# Sub-feature capability: Intent Resolution Anchor And Occasion
...
Resolve whether an outfit request is anchor-led, cart-led, or occasion-led and establish the lead intent that downstream assembly should satisfy.
...
This capability sits under `Complete-look orchestration` and owns one clear responsibility: resolve whether an outfit request is anchor-led, cart-led, or occasion-led and establish the lead intent that downstream assembly should satisfy.
...
- Prevents downstream consumers from reinterpreting `Intent Resolution Anchor And Occasion` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A PDP, cart, clienteling, or occasion surface requests an outfit recommendation.
- Multiple possible lead items or contexts appear in a single request.
- A surface promises a complete-look answer and needs a stable assembly mission.

## 5. Inputs

- normalized delivery request context
- anchor product, cart, or occasion metadata
- surface and mode context
- customer or session hints when allowed
- governance or campaign constraints

## 6. Outputs

- resolved outfit mission and lead context
- anchor or occasion scope for downstream slot filling
- multi-anchor conflict markers when present
- trace metadata showing how the mission was resolved

## 7. Workflow / Lifecycle

1. Read the incoming recommendation request and determine whether it is anchor-led, cart-led, or occasion-led.
2. Choose the primary outfit mission using explicit request data and policy precedence.
3. Record any competing lead items or unresolved ambiguities for later trace review.
4. Pass the resolved mission to template selection and candidate assembly.
5. Emit mission-resolution telemetry for analytics and debugging.
...
- outfit.intent.conflict.detected

## 13. Events Consumed

- delivery.context.normalized
- cart.context.updated
- occasion.context.resolved
...
1. A request or upstream change triggers `Intent Resolution Anchor And Occasion`.
...
```json
{
  "subFeature": "intent-resolution-anchor-and-occasion",
  "feature": "complete-look-orchestration",
  "input": "normalized delivery request context",
  "output": "resolved outfit mission and lead context",
...
- Backend services: add or extend a `intent-resolution-anchor-and-occasion` service boundary under the `complete-look-orchestration` domain.
