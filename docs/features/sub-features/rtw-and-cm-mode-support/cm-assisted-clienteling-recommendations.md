# Sub-feature capability: CM Assisted Clienteling Recommendations
...
Support CM recommendation retrieval and adaptation in assisted-selling contexts where operator explanation, configuration awareness, and governance depth are stronger than in self-service flows.
...
This capability sits under `RTW and CM mode support` and owns one clear responsibility: support cm recommendation retrieval and adaptation in assisted-selling contexts where operator explanation, configuration awareness, and governance depth are stronger than in self-service flows.
...
- Prevents downstream consumers from reinterpreting `CM Assisted Clienteling Recommendations` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A clienteling session explicitly uses CM mode.
- A stylist needs configuration-aware recommendations during an appointment.
- A CM recommendation requires operator adaptation or review before sharing.

## 5. Inputs

- clienteling session context
- CM configuration snapshot
- customer identity and appointment metadata
- governance and explanation policy
- operator role and permissions

## 6. Outputs

- CM clienteling recommendation payload
- operator explanation summary
- adaptation and share controls
- mode-aware trace and audit records

## 7. Workflow / Lifecycle

1. Prepare the clienteling recommendation request with CM and appointment context.
2. Validate the configuration snapshot for assisted use.
3. Retrieve the recommendation set and expose operator-safe explanation detail.
4. Allow governed adaptation and sharing flows when policy permits.
5. Record the session outcome for analytics, audit, and later customer support.
...
- CMClientelingRecommendation
- AssistedUseValidation
- AppointmentRecommendationContext
- CMOperatorExplanation
- ClientelingAuditBundle
...
- POST /clienteling/recommendations
- POST /clienteling/adaptations
...
- cm.clienteling.recommendation.delivered
- cm.clienteling.adaptation.saved

## 13. Events Consumed

- cm.configuration.validated
- clienteling.recommendation.retrieved
...
1. A request or upstream change triggers `CM Assisted Clienteling Recommendations`.
...
```json
{
  "subFeature": "cm-assisted-clienteling-recommendations",
  "feature": "rtw-and-cm-mode-support",
  "input": "clienteling session context",
  "output": "CM clienteling recommendation payload",
...
- Backend services: add or extend a `cm-assisted-clienteling-recommendations` service boundary under the `rtw-and-cm-mode-support` domain.
