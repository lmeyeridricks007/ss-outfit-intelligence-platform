# Sub-feature capability: Governance And Versioned Snapshot Linkage
...
Link traces to versioned governance and recommendation snapshots so investigators can see exactly which controls and payloads shaped the delivered outcome.
...
This capability sits under `Explainability and auditability` and owns one clear responsibility: link traces to versioned governance and recommendation snapshots so investigators can see exactly which controls and payloads shaped the delivered outcome.
...
- Prevents downstream consumers from reinterpreting `Governance And Versioned Snapshot Linkage` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A recommendation trace is written or retrieved.
- A governance snapshot or delivery snapshot changes after a recommendation is delivered.
- An audit needs control-level lineage for a recommendation set.

## 5. Inputs

- trace record
- governance snapshot ID and control IDs
- delivery snapshot references
- publication timestamps
- snapshot retention and visibility rules

## 6. Outputs

- trace-linked governance context
- trace-linked delivery snapshot references
- version-aware reconstruction metadata
- control-change comparison records

## 7. Workflow / Lifecycle

1. Attach the effective governance snapshot and control identifiers to the trace record.
2. Attach the immutable delivery snapshot reference used for consumer output.
3. Preserve version lineage even if later governance state changes.
4. Expose comparisons between historical and current effective state when requested.
5. Keep the linkage retrievable for audit, support, and analytics consumers.
...
- TraceGovernanceContext
- SnapshotLinkRecord
- HistoricalComparisonRecord
- ControlLineageReference
- TraceSnapshotBundle
...
- GET /traces/{traceId}/governance
- GET /traces/{traceId}/snapshots
...
1. A request or upstream change triggers `Governance And Versioned Snapshot Linkage`.
...
```json
{
  "subFeature": "governance-and-versioned-snapshot-linkage",
  "feature": "explainability-and-auditability",
  "input": "trace record",
  "output": "trace-linked governance context",
...
- Backend services: add or extend a `governance-and-versioned-snapshot-linkage` service boundary under the `explainability-and-auditability` domain.
