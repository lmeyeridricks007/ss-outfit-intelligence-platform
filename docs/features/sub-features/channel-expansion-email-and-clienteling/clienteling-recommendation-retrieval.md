...
Retrieve recommendation sets for authenticated clienteling sessions with appointment, customer, and anchor context while preserving shared IDs, freshness, and traceability.
...
This capability sits under `Channel expansion: email and clienteling` and owns one clear responsibility: retrieve recommendation sets for authenticated clienteling sessions with appointment, customer, and anchor context while preserving shared ids, freshness, and traceability.
...
- Prevents downstream consumers from reinterpreting `Clienteling Recommendation Retrieval` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A stylist or associate opens a clienteling session.
- A clienteling user refreshes recommendations during a live appointment.
- An appointment or client context changes materially.

## 5. Inputs

- authenticated operator and customer context
- appointment or store context
- anchor products or occasion notes
- identity confidence and profile state
- clienteling surface policy

## 6. Outputs

- clienteling session recommendation payload
- stable recommendation-set and trace references
- refresh and recency metadata
- operator-safe explanation summaries

## 7. Workflow / Lifecycle

1. Build the shared recommendation request using clienteling session context.
2. Request recommendations and receive the shared delivery response for the clienteling surface.
3. Render or expose the result through clienteling-safe view models.
4. Refresh the recommendations when context changes materially, issuing new recommendation-set IDs.
5. Persist trace, session, and operator access linkage for later audit.
...
- ClientelingSessionRecommendation
- AppointmentContext
- SessionRefreshRecord
- OperatorAccessContext
- ClientelingViewModel
...
- POST /clienteling/recommendations/{sessionId}/refresh
...
- clienteling.recommendation.refreshed
...
1. A request or upstream change triggers `Clienteling Recommendation Retrieval`.
...
```json
{
  "subFeature": "clienteling-recommendation-retrieval",
  "feature": "channel-expansion-email-and-clienteling",
  "input": "authenticated operator and customer context",
  "output": "clienteling session recommendation payload",
...
- Backend services: add or extend a `clienteling-recommendation-retrieval` service boundary under the `channel-expansion-email-and-clienteling` domain.
