# Sub-feature capability: Governance Campaign And Ordering Freedom
...
Apply governance precedence, campaigns, overrides, and curated ordering-freedom rules so the remaining candidate pool respects operator intent before any scoring logic runs.
...
This capability sits under `Recommendation decisioning and ranking` and owns one clear responsibility: apply governance precedence, campaigns, overrides, and curated ordering-freedom rules so the remaining candidate pool respects operator intent before any scoring logic runs.
...
- Prevents downstream consumers from reinterpreting `Governance Campaign And Ordering Freedom` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- An eligible candidate pool is ready for precedence evaluation.
- Campaigns, pins, overrides, or curated looks affect ordering rules.
- A surface or mode uses different ordering freedom than another.

## 5. Inputs

- eligible candidate pool
- governance snapshot
- campaign and override records
- surface and mode policy
- curated ordering-freedom settings

## 6. Outputs

- governed candidate pool
- ordering constraints and fixed positions
- campaign and override annotations
- trace-ready precedence decisions

## 7. Workflow / Lifecycle

1. Read the current governance snapshot and determine applicable hard and soft controls.
2. Apply fixed ordering, pins, boosts, suppressions, and campaign overlays in precedence order.
3. Mark which candidates may still be reordered by ranking logic.
4. Return the governed pool and explicit ordering constraints.
5. Emit governance-precedence telemetry for later audit and analysis.
...
- POST /decisioning/governance/apply
- GET /decisioning/governance/{snapshotId}
...
- decisioning.ordering-freedom.resolved
...
1. A request or upstream change triggers `Governance Campaign And Ordering Freedom`.
...
```json
{
  "subFeature": "governance-campaign-and-ordering-freedom",
  "feature": "recommendation-decisioning-and-ranking",
  "input": "eligible candidate pool",
  "output": "governed candidate pool",
...
- Backend services: add or extend a `governance-campaign-and-ordering-freedom` service boundary under the `recommendation-decisioning-and-ranking` domain.
