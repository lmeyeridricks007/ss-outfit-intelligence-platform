# Governance Eligibility Ranking Assembly
...
- **Sub-feature slug:** `governance-eligibility-ranking-assembly`
...
Apply eligibility, governance, ranking, and set assembly in the required order so downstream responses remain safe and explainable.

## 2. Core Concept

Governance Eligibility Ranking Assembly is the capability slice of **Recommendation Orchestration and Types** that turns the parent feature intent into a bounded implementation contract with explicit lifecycle, contracts, and operational behavior.

## 3. User Problems Solved

- Downstream teams need governance eligibility ranking assembly behavior that is explicitly defined instead of inferred from the parent feature doc.
- Recommendation-serving paths need implementation-ready contracts for governance eligibility ranking assembly so engineering does not guess at lifecycle or data semantics.
...
- Candidate collection completes
- A governance snapshot changes
- A ranking policy is updated

## 5. Inputs

- Candidate pool with provenance
- Eligibility and governance decisions
- Ranking models and rules

## 6. Outputs

- Assembled recommendation set
- Assembly decision trace metadata
- Degraded-mode annotations

## 7. Workflow / Lifecycle

1. Filter candidates through hard eligibility constraints.
2. Apply governance directives and then ranking logic to the surviving pool.
3. Assemble the final recommendation set with explicit decision metadata.

## 8. Business Rules

- Hard eligibility and governance constraints must run before ranking.
- Ranking cannot reintroduce excluded candidates.
- Assembly decisions need stable identifiers for analytics and explainability.

## 9. Configuration Model

- Governance Eligibility Ranking Assembly policy version, rollout state, and owner metadata.
...
- `governance_eligibility_ranking_assembly` primary record storing the current governance eligibility ranking assembly state.
- `governance_eligibility_ranking_assembly_history` append-only history for lifecycle and trace reconstruction.
- `governance_eligibility_ranking_assembly_projection` read model optimized for downstream serving or operator lookup.
...
- POST /internal/recommendations/assemble
- GET /internal/recommendations/assembly-policies
...
- Assembly step ladder
- Decision rationale chip

## 16. UI Screens

- Assembly trace panel
- Ranking policy review page
...
- Metrics: request count, success rate, degraded rate, and freshness for governance eligibility ranking assembly.
...
1. API flow: call `POST /internal/recommendations/assemble` with `Candidate pool with provenance` and return `Assembled recommendation set` plus traceable metadata.
2. Event flow: consume `RecommendationCandidatesCollected` and emit `RecommendationSetAssembled` after business rules and lifecycle checks pass.
3. Operator flow: use `Assembly trace panel` to inspect or change governance eligibility ranking assembly behavior, then verify the outcome in the linked trace or health view.

## 23. Implementation Notes

- Backend services involved: `recommendation-orchestration-and-types-service`, `recommendation-orchestration-and-types-api`, and `governance-eligibility-ranking-assembly-worker` for async processing.
- Database tables or collections required: `governance_eligibility_ranking_assembly`, `governance_eligibility_ranking_assembly_history`, and `governance_eligibility_ranking_assembly_projection`.
- Jobs or workers required: `governance-eligibility-ranking-assembly-worker`, nightly reconciliation, and replay tooling for operational recovery.
- External APIs or systems required: Merchandising governance, Analytics and experimentation, Explainability trace service.
- Frontend components required: Assembly step ladder, Decision rationale chip.
...
- How should campaigns and customer relevance trade off on different surfaces?
- What minimum model confidence is needed before falling back to curated sources?
