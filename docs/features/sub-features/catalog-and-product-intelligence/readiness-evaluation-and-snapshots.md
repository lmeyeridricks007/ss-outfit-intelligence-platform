...
Evaluate multi-dimensional recommendation readiness and persist snapshots that explain whether a product is eligible, degraded, suppressed, or blocked for recommendation use.
...
This capability sits under `Catalog and product intelligence` and owns one clear responsibility: evaluate multi-dimensional recommendation readiness and persist snapshots that explain whether a product is eligible, degraded, suppressed, or blocked for recommendation use.
...
- Prevents downstream consumers from reinterpreting `Readiness Evaluation And Snapshots` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A product identity or attribute change affects recommendation safety.
- Inventory, imagery, compatibility, or mode-specific evidence changes.
- Operators or downstream services request the latest readiness status.

## 5. Inputs

- canonical product records
- inventory, imagery, compatibility, and freshness evidence
- readiness policy version
- market and mode eligibility context
- manual suppressions or overrides

## 6. Outputs

- versioned `ProductReadinessSnapshot`
- dimension-level readiness status and reason codes
- product-level degraded or suppression state
- projection-friendly readiness summaries

## 7. Workflow / Lifecycle

1. Load the current canonical product record and all supporting evidence.
2. Evaluate each readiness dimension independently according to policy.
3. Compose the final product readiness state and attach reason codes.
4. Persist a versioned snapshot and mark downstream projection updates.
5. Expose the snapshot for delivery, ranking, governance, and operator use.
...
- GET /catalog/products/{canonicalProductId}/readiness
...
1. A request or upstream change triggers `Readiness Evaluation And Snapshots`.
...
```json
{
  "subFeature": "readiness-evaluation-and-snapshots",
  "feature": "catalog-and-product-intelligence",
  "input": "canonical product records",
  "output": "versioned ProductReadinessSnapshot",
...
- Backend services: add or extend a `readiness-evaluation-and-snapshots` service boundary under the `catalog-and-product-intelligence` domain.
