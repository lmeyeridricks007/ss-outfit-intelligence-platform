# Sub-feature capability: Canonical Identity Resolution
...
Resolve stable canonical customer IDs from source-specific identifiers with explicit confidence, provenance, and lifecycle state so recommendation systems can reuse identity safely across channels.
...
This capability sits under `Identity and style profile` and owns one clear responsibility: resolve stable canonical customer ids from source-specific identifiers with explicit confidence, provenance, and lifecycle state so recommendation systems can reuse identity safely across channels.
...
- Prevents downstream consumers from reinterpreting `Canonical Identity Resolution` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A new login, order, loyalty, CRM, or store identifier is observed.
- Identity linkage confidence changes after new evidence appears.
- A downstream system requests canonical customer identity for recommendation use.

## 5. Inputs

- source-system customer identifiers
- identity-link evidence and confidence signals
- source precedence and auto-link policy
- existing canonical identity records
- sensitive-linkage review rules
...
- conflict or review cases when linkage is unsafe

## 7. Workflow / Lifecycle

1. Collect identity evidence from approved sources and compare it to existing canonical mappings.
2. Score the match according to linkage and confidence policy.
3. Create, update, or withhold a canonical link based on the evidence strength.
4. Persist confidence, provenance, and lifecycle state for the identity mapping.
5. Expose the resulting identity state to profile, ranking, delivery, and review workflows.
...
- POST /identity/link
- GET /identity/{canonicalCustomerId}
...
1. A request or upstream change triggers `Canonical Identity Resolution`.
...
```json
{
  "subFeature": "canonical-identity-resolution",
  "feature": "identity-and-style-profile",
  "input": "source-system customer identifiers",
  "output": "canonical customer ID",
...
- Backend services: add or extend a `canonical-identity-resolution` service boundary under the `identity-and-style-profile` domain.
