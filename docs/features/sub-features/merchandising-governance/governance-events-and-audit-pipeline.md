...
Emit governance events and immutable audit records so recommendation traces can explain who changed what, when, and why.

## 2. Core Concept

Governance Events and Audit Pipeline is the capability slice of **Merchandising Governance** that turns the parent feature intent into a bounded implementation contract with explicit lifecycle, contracts, and operational behavior.

## 3. User Problems Solved

- Downstream teams need governance events and audit pipeline behavior that is explicitly defined instead of inferred from the parent feature doc.
- Recommendation-serving paths need implementation-ready contracts for governance events and audit pipeline so engineering does not guess at lifecycle or data semantics.
...
- A governance object changes state
- An override is activated or expires
- An audit export or review is requested
...
1. Capture governance mutations and related operator context.
2. Persist immutable audit records and emit domain events.
3. Expose audit lookup and export interfaces for trace and compliance consumers.

## 8. Business Rules

- Audit records must not be rewritten after publication.
- Event payloads require stable object and version identifiers.
- Exports must respect role and retention constraints.

## 9. Configuration Model

- Governance Events and Audit Pipeline policy version, rollout state, and owner metadata.
...
- `governance_events_and_audit_pipeline` primary record storing the current governance events and audit pipeline state.
- `governance_events_and_audit_pipeline_history` append-only history for lifecycle and trace reconstruction.
- `governance_events_and_audit_pipeline_projection` read model optimized for downstream serving or operator lookup.
...
- GET /internal/governance/audits/{auditId}
- POST /internal/governance/audits/export

## 12. Events Produced

- GovernanceAuditRecorded
- GovernanceAuditExportRequested
...
- Metrics: request count, success rate, degraded rate, and freshness for governance events and audit pipeline.
...
1. API flow: call `GET /internal/governance/audits/{auditId}` with `Governance lifecycle events` and return `Immutable governance audit record` plus traceable metadata.
2. Event flow: consume `GovernanceObjectPublished` and emit `GovernanceAuditRecorded` after business rules and lifecycle checks pass.
3. Operator flow: use `Governance audit explorer` to inspect or change governance events and audit pipeline behavior, then verify the outcome in the linked trace or health view.

## 23. Implementation Notes

- Backend services involved: `merchandising-governance-service`, `merchandising-governance-api`, and `governance-events-and-audit-pipeline-worker` for async processing.
- Database tables or collections required: `governance_events_and_audit_pipeline`, `governance_events_and_audit_pipeline_history`, and `governance_events_and_audit_pipeline_projection`.
- Jobs or workers required: `governance-events-and-audit-pipeline-worker`, nightly reconciliation, and replay tooling for operational recovery.
- External APIs or systems required: Explainability trace service, Analytics and experimentation, Compliance export tooling.
- Frontend components required: Audit timeline table, Export scope selector.
...
- Which audit fields are mandatory for all governance object types?
- How long must governance history remain directly queryable versus archived?
