# Sub-feature capability: Scoring Rank And Deterministic Post Processing
...
Score the governed candidate pool, rank it according to the active objective, and then apply deterministic tie-breaking, de-duplication, and quality thresholds before delivery.
...
This capability sits under `Recommendation decisioning and ranking` and owns one clear responsibility: score the governed candidate pool, rank it according to the active objective, and then apply deterministic tie-breaking, de-duplication, and quality thresholds before delivery.
...
- Prevents downstream consumers from reinterpreting `Scoring Rank And Deterministic Post Processing` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A governed candidate pool is available for ordering.
- The request uses model-based or deterministic ranking depending on mission and policy.
- Post-processing rules are required to stabilize delivery-safe output order.

## 5. Inputs

- governed candidate pool
- ranking objective and feature inputs
- model or deterministic scorer configuration
- duplicate-suppression and tie-break rules
- quality threshold policy

## 6. Outputs

- ordered recommendation candidates
- score and explanation summaries
- post-processing adjustment records
- delivery-safe final ordering

## 7. Workflow / Lifecycle

1. Calculate scores for the governed candidates using the configured scorer path.
2. Order the candidates according to mission-specific objectives and constraints.
3. Apply deterministic post-processing such as de-duplication, tie-breaking, or thin-pool cleanup.
4. Persist the final ranking result and the adjustments that were made.
5. Publish the final ordering to delivery and trace systems.
...
- decisioning.post-processing.applied
...
1. A request or upstream change triggers `Scoring Rank And Deterministic Post Processing`.
...
```json
{
  "subFeature": "scoring-rank-and-deterministic-post-processing",
  "feature": "recommendation-decisioning-and-ranking",
  "input": "governed candidate pool",
  "output": "ordered recommendation candidates",
...
- Backend services: add or extend a `scoring-rank-and-deterministic-post-processing` service boundary under the `recommendation-decisioning-and-ranking` domain.
