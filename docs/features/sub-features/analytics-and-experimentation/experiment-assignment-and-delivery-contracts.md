...
Assign experiments and expose experiment context to delivery, ranking, analytics, and support tooling so treatment effects remain traceable across recommendation outcomes.
...
This capability sits under `Analytics and experimentation` and owns one clear responsibility: assign experiments and expose experiment context to delivery, ranking, analytics, and support tooling so treatment effects remain traceable across recommendation outcomes.
...
- Prevents downstream consumers from reinterpreting `Experiment Assignment And Delivery Contracts` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A request or user qualifies for an active experiment.
- A new experiment or holdout is launched for recommendation experiences.
- Delivery or ranking needs the experiment assignment context for a response.

## 5. Inputs

- experiment definitions and targeting rules
- request identity or session key
- surface, market, and recommendation-type scope
- assignment and holdout policy
- active rollout configuration

## 6. Outputs

- experiment assignment record
- variant and holdout metadata
- delivery-facing experiment context
- assignment audit and stickiness records

## 7. Workflow / Lifecycle

1. Evaluate the request or subject against active experiment targeting rules.
2. Assign the subject to a variant or holdout using the configured key.
3. Attach assignment metadata to delivery, ranking, and telemetry payloads.
4. Persist stickiness and assignment lineage for later analysis.
5. Emit assignment events for monitoring and experiment scorecards.
...
- GET /analytics/experiments/{experimentId}
...
1. A request or upstream change triggers `Experiment Assignment And Delivery Contracts`.
...
```json
{
  "subFeature": "experiment-assignment-and-delivery-contracts",
  "feature": "analytics-and-experimentation",
  "input": "experiment definitions and targeting rules",
  "output": "experiment assignment record",
...
- Backend services: add or extend a `experiment-assignment-and-delivery-contracts` service boundary under the `analytics-and-experimentation` domain.
