# Sub-feature capability: Style Profile Snapshot Computation
...
Compute immutable style profile snapshots that summarize recommendation-relevant preferences, affinities, and ownership signals without exposing raw source history directly.
...
This capability sits under `Identity and style profile` and owns one clear responsibility: compute immutable style profile snapshots that summarize recommendation-relevant preferences, affinities, and ownership signals without exposing raw source history directly.
...
- Prevents downstream consumers from reinterpreting `Style Profile Snapshot Computation` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A significant set of customer signals or mappings changes.
- A scheduled profile recomputation runs.
- A downstream consumer requests the latest valid profile snapshot.

## 5. Inputs

- identity-resolved customer records
- eligible and freshness-classified customer signals
- profile domain policy
- suppression and ownership rules
- snapshot recomputation schedule

## 6. Outputs

- immutable profile snapshot
- domain-level freshness and confidence summaries
- snapshot lineage to source signals
- snapshot state markers such as stale or partially computed

## 7. Workflow / Lifecycle

1. Load the eligible signals and identity record for the target customer.
2. Compute allowed profile domains such as affinities, wardrobe signals, or occasion tendencies.
3. Persist the profile snapshot with freshness, lineage, and policy references.
4. Mark the snapshot state as current, stale, partial, or expired as needed.
5. Publish the snapshot reference for activation-envelope and ranking use.
...
- ProfileSnapshot
- ProfileDomainSummary
- ProfileLineageLink
- SnapshotState
- ProfileFreshnessRecord
...
- GET /profiles/snapshots/{profileSnapshotId}
...
1. A request or upstream change triggers `Style Profile Snapshot Computation`.
...
```json
{
  "subFeature": "style-profile-snapshot-computation",
  "feature": "identity-and-style-profile",
  "input": "identity-resolved customer records",
  "output": "immutable profile snapshot",
...
- Backend services: add or extend a `style-profile-snapshot-computation` service boundary under the `identity-and-style-profile` domain.
