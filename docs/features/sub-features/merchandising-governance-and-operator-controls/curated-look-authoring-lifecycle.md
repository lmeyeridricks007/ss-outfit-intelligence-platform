# Sub-feature capability: Curated Look Authoring Lifecycle
...
Support the full lifecycle for curated look authoring, versioning, scheduling, and retirement so editorial and merchandising teams can shape recommendation meaning intentionally.
...
This capability sits under `Merchandising governance and operator controls` and owns one clear responsibility: support the full lifecycle for curated look authoring, versioning, scheduling, and retirement so editorial and merchandising teams can shape recommendation meaning intentionally.
...
- Prevents downstream consumers from reinterpreting `Curated Look Authoring Lifecycle` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A merchandiser creates or edits a curated look.
- A look version needs approval, scheduling, publication, or retirement.
- Decisioning or preview tooling needs the current effective curated look set.

## 5. Inputs

- curated look members and ordering rules
- market, channel, and mode scope
- effective dates and scheduling metadata
- authoring notes and rationale
- approval or publication status

## 6. Outputs

- versioned curated look records
- effective publication schedule
- preview and diff artifacts
- decisioning-consumable curated look references

## 7. Workflow / Lifecycle

1. Create or revise the curated look in draft state with member and ordering details.
2. Validate the look against canonical product and scope rules.
3. Route the change through approval and scheduling logic.
4. Publish a new effective look version and retire superseded versions safely.
5. Update governance snapshots and operator history views.
...
- POST /governance/curated-looks
- POST /governance/curated-looks/{lookId}/publish
- GET /governance/curated-looks/{lookId}/preview
...
- governance.curated-look.retired
...
1. A request or upstream change triggers `Curated Look Authoring Lifecycle`.
...
```json
{
  "subFeature": "curated-look-authoring-lifecycle",
  "feature": "merchandising-governance-and-operator-controls",
  "input": "curated look members and ordering rules",
  "output": "versioned curated look records",
...
- Backend services: add or extend a `curated-look-authoring-lifecycle` service boundary under the `merchandising-governance-and-operator-controls` domain.
