# Sub-feature capability: Market Channel And Mode Eligibility
...
Compute market, channel, and mode eligibility so products only enter recommendation pools where assortment, policy, and customer-facing promises actually allow them to appear.
...
This capability sits under `Catalog and product intelligence` and owns one clear responsibility: compute market, channel, and mode eligibility so products only enter recommendation pools where assortment, policy, and customer-facing promises actually allow them to appear.
...
- Prevents downstream consumers from reinterpreting `Market Channel And Mode Eligibility` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A product becomes sellable or unsellable in a market or channel.
- A surface-specific or mode-specific policy changes.
- Decisioning asks for the eligible candidate pool for a request scope.

## 5. Inputs

- canonical product and readiness snapshot
- market and channel assortment rules
- mode-specific policy settings
- surface eligibility constraints
- manual suppression or exception records

## 6. Outputs

- assortment eligibility records
- surface- and mode-scoped inclusion or exclusion decisions
- eligible candidate projections
- explanatory reason codes

## 7. Workflow / Lifecycle

1. Read the latest product readiness snapshot and current assortment rules.
2. Apply market, channel, surface, and mode filters in deterministic order.
3. Persist the effective eligibility record for low-latency downstream use.
4. Emit changes when the product enters or leaves a recommendation-safe pool.
5. Surface unresolved policy conflicts for operator follow-up.
...
- SurfaceEligibilityRule
- ModeEligibilityRecord
...
- GET /catalog/products/{canonicalProductId}/eligibility
- POST /catalog/eligibility/recompute
...
1. A request or upstream change triggers `Market Channel And Mode Eligibility`.
...
```json
{
  "subFeature": "market-channel-and-mode-eligibility",
  "feature": "catalog-and-product-intelligence",
  "input": "canonical product and readiness snapshot",
  "output": "assortment eligibility records",
...
- Backend services: add or extend a `market-channel-and-mode-eligibility` service boundary under the `catalog-and-product-intelligence` domain.
