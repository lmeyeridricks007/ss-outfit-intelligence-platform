# Sub-feature capability: Merchandising Rules And Precedence
...
Author and apply merchandising rules with explicit precedence so suppressions, inclusions, boosts, pins, and exclusions are deterministic before ranking begins.
...
This capability sits under `Merchandising governance and operator controls` and owns one clear responsibility: author and apply merchandising rules with explicit precedence so suppressions, inclusions, boosts, pins, and exclusions are deterministic before ranking begins.
...
- Prevents downstream consumers from reinterpreting `Merchandising Rules And Precedence` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- An operator defines or updates a merchandising rule.
- A conflicting set of rules must be reconciled before publication.
- Decisioning needs the effective ordered rule set for a request scope.

## 5. Inputs

- rule definition and type
- scope dimensions such as market, surface, channel, recommendation type, and mode
- priority and hardness settings
- effective dates and approval state
- linked product, category, look, or segment references

## 6. Outputs

- versioned merchandising rule set
- conflict warnings and precedence decisions
- effective rule evaluation order
- decisioning-facing rule projections

## 7. Workflow / Lifecycle

1. Create or edit the merchandising rule with scope, hardness, and priority.
2. Validate references and detect precedence conflicts against existing rules.
3. Resolve or escalate conflicts before publication.
4. Publish the effective rule set and update governance snapshots.
5. Record the rule lineage for future audit and trace use.
...
- GET /governance/rules/effective
- POST /governance/rules/{ruleId}/validate
...
1. A request or upstream change triggers `Merchandising Rules And Precedence`.
...
```json
{
  "subFeature": "merchandising-rules-and-precedence",
  "feature": "merchandising-governance-and-operator-controls",
  "input": "rule definition and type",
  "output": "versioned merchandising rule set",
...
- Backend services: add or extend a `merchandising-rules-and-precedence` service boundary under the `merchandising-governance-and-operator-controls` domain.
