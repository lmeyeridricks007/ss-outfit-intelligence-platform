# Sub-feature capability: Identity Conflicts And Operator Review
...
Surface conflicted or sensitive identity cases to operators so unsafe auto-linking does not silently distort personalization, suppression, or attribution behavior.
...
This capability sits under `Identity and style profile` and owns one clear responsibility: surface conflicted or sensitive identity cases to operators so unsafe auto-linking does not silently distort personalization, suppression, or attribution behavior.
...
- Prevents downstream consumers from reinterpreting `Identity Conflicts And Operator Review` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- Auto-linking rules cannot confidently reconcile identity evidence.
- A high-risk merge, split, or restricted profile case requires review.
- Support or governance users investigate identity anomalies affecting recommendations.

## 5. Inputs

- identity conflict signals
- source evidence and provenance
- operator role and review policy
- sensitive-case escalation rules
- affected profile and suppression context

## 6. Outputs

- identity review cases
- operator decisions and escalation records
- updated identity linkage status
- audit history for conflict handling

## 7. Workflow / Lifecycle

1. Create a review case when identity evidence is conflicted or sensitive.
2. Show operators the relevant source evidence and policy constraints.
3. Capture approval, rejection, split, or escalation outcomes.
4. Propagate the resulting linkage decision back to profile and recommendation consumers.
5. Retain the full review history for audit and explainability.
...
- GET /identity/cases
- POST /identity/cases/{caseId}/resolve
...
1. A request or upstream change triggers `Identity Conflicts And Operator Review`.
...
```json
{
  "subFeature": "identity-conflicts-and-operator-review",
  "feature": "identity-and-style-profile",
  "input": "identity conflict signals",
  "output": "identity review cases",
...
- Backend services: add or extend a `identity-conflicts-and-operator-review` service boundary under the `identity-and-style-profile` domain.
