# Sub-feature capability: Phase 2 Homepage Inspiration Occasion Surfaces
...
Extend web recommendation experiences beyond PDP and cart to homepage, inspiration, and occasion-led surfaces while preserving the same delivery contract and recommendation-type semantics.
...
This capability sits under `Ecommerce surface experiences` and owns one clear responsibility: extend web recommendation experiences beyond pdp and cart to homepage, inspiration, and occasion-led surfaces while preserving the same delivery contract and recommendation-type semantics.
...
- Prevents downstream consumers from reinterpreting `Phase 2 Homepage Inspiration Occasion Surfaces` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A later-phase web placement is enabled.
- Homepage or inspiration surfaces need recommendation-driven modules.
- Occasion-led entry points require shared contract support.

## 5. Inputs

- placement registry configuration
- delivery context for homepage or occasion surfaces
- campaign and context policy
- surface-specific UX copy and design
- analytics instrumentation requirements

## 6. Outputs

- homepage or inspiration module configurations
- surface-specific module view models
- expanded telemetry coverage
- phase-specific rollout controls

## 7. Workflow / Lifecycle

1. Enable the target surface through placement and rollout configuration.
2. Build the appropriate request context for the new surface.
3. Render the returned recommendation types with surface-appropriate modules.
4. Instrument the new placements using the shared analytics event model.
5. Track rollout quality and degraded-state behavior before wider expansion.
...
- ExpandedSurfacePlacement
- OccasionSurfaceContext
- RolloutFlag
- InspirationModuleViewModel
- SurfaceAnalyticsBundle
...
- GET /placements/{placementId}/config
...
- surface.expansion.enabled
- surface.expansion.rendered
...
1. A request or upstream change triggers `Phase 2 Homepage Inspiration Occasion Surfaces`.
...
```json
{
  "subFeature": "phase-2-homepage-inspiration-occasion-surfaces",
  "feature": "ecommerce-surface-experiences",
  "input": "placement registry configuration",
  "output": "homepage or inspiration module configurations",
...
- Backend services: add or extend a `phase-2-homepage-inspiration-occasion-surfaces` service boundary under the `ecommerce-surface-experiences` domain.
