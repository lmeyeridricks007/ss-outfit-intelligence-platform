...
- **Sub-feature slug:** `catalog-quality-monitoring`
...
Track catalog quality, feed freshness, mapping conflicts, and recommendation-readiness so degraded modes are visible and actionable.

## 2. Core Concept

Catalog Quality Monitoring is the capability slice of **Catalog and Eligibility Foundation** that turns the parent feature intent into a bounded implementation contract with explicit lifecycle, contracts, and operational behavior.

## 3. User Problems Solved

- Downstream teams need catalog quality monitoring behavior that is explicitly defined instead of inferred from the parent feature doc.
- Recommendation-serving paths need implementation-ready contracts for catalog quality monitoring so engineering does not guess at lifecycle or data semantics.
...
- Feed freshness or completeness falls below thresholds
- Validation failures spike for a market or source
- Operators need a current view of catalog health

## 5. Inputs

- Catalog lifecycle metrics
- Validation and mapping error logs
- SLO definitions and alert thresholds

## 6. Outputs

- Catalog health dashboards
- Quality alerts and incident signals
- Degraded mode annotations for downstream consumers

## 7. Workflow / Lifecycle

1. Aggregate quality signals from ingestion, validation, and read models.
2. Compare health metrics with alerting and SLO policies.
3. Notify operators and mark downstream consumers when catalog health is degraded.

## 8. Business Rules

- Quality monitoring must distinguish data defects from pipeline outages.
- Degraded mode annotations need explicit start and end timestamps.
- Alerts require stable reason categories to support incident trend analysis.

## 9. Configuration Model

- Catalog Quality Monitoring policy version, rollout state, and owner metadata.
...
- `catalog_quality_monitoring` primary record storing the current catalog quality monitoring state.
- `catalog_quality_monitoring_history` append-only history for lifecycle and trace reconstruction.
- `catalog_quality_monitoring_projection` read model optimized for downstream serving or operator lookup.
...
- GET /internal/catalog/health
- GET /internal/catalog/quality/markets/{marketCode}

## 12. Events Produced

- CatalogHealthDegraded
- CatalogHealthRecovered
...
- Recommendation orchestration health checks
...
- Metrics: request count, success rate, degraded rate, and freshness for catalog quality monitoring.
...
1. API flow: call `GET /internal/catalog/health` with `Catalog lifecycle metrics` and return `Catalog health dashboards` plus traceable metadata.
2. Event flow: consume `CatalogIngestionRunCompleted` and emit `CatalogHealthDegraded` after business rules and lifecycle checks pass.
3. Operator flow: use `Catalog health dashboard` to inspect or change catalog quality monitoring behavior, then verify the outcome in the linked trace or health view.

## 23. Implementation Notes

- Backend services involved: `catalog-and-eligibility-foundation-service`, `catalog-and-eligibility-foundation-api`, and `catalog-quality-monitoring-worker` for async processing.
- Database tables or collections required: `catalog_quality_monitoring`, `catalog_quality_monitoring_history`, and `catalog_quality_monitoring_projection`.
- Jobs or workers required: `catalog-quality-monitoring-worker`, nightly reconciliation, and replay tooling for operational recovery.
- External APIs or systems required: Alerting and on-call system, Observability dashboards, Recommendation orchestration health checks.
- Frontend components required: Health status banner, Quality trend chart.
...
- What degraded thresholds should be surface-specific versus global?
- How should catalog health be summarized for non-technical operators?
