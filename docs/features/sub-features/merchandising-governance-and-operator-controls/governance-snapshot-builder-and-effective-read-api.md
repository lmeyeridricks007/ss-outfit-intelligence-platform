# Sub-feature capability: Governance Snapshot Builder And Effective Read API
...
Build immutable effective governance snapshots and expose read APIs that decisioning, delivery, explainability, and support tooling can use consistently.
...
This capability sits under `Merchandising governance and operator controls` and owns one clear responsibility: build immutable effective governance snapshots and expose read apis that decisioning, delivery, explainability, and support tooling can use consistently.
...
- Prevents downstream consumers from reinterpreting `Governance Snapshot Builder And Effective Read API` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- Any curated look, rule, campaign, or override becomes effective.
- A downstream service requests the effective governance state for a scope.
- Cache invalidation or trace reconstruction needs the exact snapshot used in delivery.

## 5. Inputs

- published governance family records
- scope and timing filters
...
- snapshot version and change markers
- read-optimized governance projection
- snapshot lineage for traces and analytics

## 7. Workflow / Lifecycle

1. Collect all currently effective governance controls for the requested scope and time.
2. Resolve precedence across rule families and ordering-freedom constraints.
3. Persist a new immutable governance snapshot when the effective state changes.
4. Expose read APIs and projections for downstream consumers.
5. Notify caches and analytics systems that the effective snapshot has changed.
...
- POST /governance/snapshots/rebuild
...
- governance.snapshot.invalidated
...
1. A request or upstream change triggers `Governance Snapshot Builder And Effective Read API`.
...
```json
{
  "subFeature": "governance-snapshot-builder-and-effective-read-api",
  "feature": "merchandising-governance-and-operator-controls",
  "input": "published governance family records",
  "output": "effective governance snapshot",
...
- Backend services: add or extend a `governance-snapshot-builder-and-effective-read-api` service boundary under the `merchandising-governance-and-operator-controls` domain.
