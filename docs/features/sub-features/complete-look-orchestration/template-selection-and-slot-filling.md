# Sub-feature capability: Template Selection And Slot Filling
...
Select the correct outfit template and fill required and optional slots so complete-look answers remain structured, market-aware, and mode-aware instead of opportunistically grouped.
...
This capability sits under `Complete-look orchestration` and owns one clear responsibility: select the correct outfit template and fill required and optional slots so complete-look answers remain structured, market-aware, and mode-aware instead of opportunistically grouped.
...
- Prevents downstream consumers from reinterpreting `Template Selection And Slot Filling` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- An outfit mission is resolved and candidate relationships are available.
- Surface or mode policy requires a different slot template.
- A curated look or compatibility graph can seed grouped assembly.

## 5. Inputs

- resolved outfit mission
- candidate products and compatibility edges
- template policy by market, surface, and mode
- curated look data and ranking hints
- slot requirement rules

## 6. Outputs

- selected assembly template
- slot-to-product assignments
- unfilled or substituted slot markers
- group-ready outfit assembly record

## 7. Workflow / Lifecycle

1. Choose the assembly template that matches the mission, surface, market, and mode.
2. Populate required slots first and then optional slots from available candidates.
3. Validate slot-to-product compatibility and preserve anchor semantics where promised.
4. Mark substitutions, omissions, or unresolved slots with explicit metadata.
5. Return the structured outfit assembly for validation and delivery packaging.
...
- GET /outfit-templates/{templateId}
...
1. A request or upstream change triggers `Template Selection And Slot Filling`.
...
```json
{
  "subFeature": "template-selection-and-slot-filling",
  "feature": "complete-look-orchestration",
  "input": "resolved outfit mission",
  "output": "selected assembly template",
...
- Backend services: add or extend a `template-selection-and-slot-filling` service boundary under the `complete-look-orchestration` domain.
