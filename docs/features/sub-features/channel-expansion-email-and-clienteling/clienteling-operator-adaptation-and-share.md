# Sub-feature capability: Clienteling Operator Adaptation And Share
...
Allow clienteling operators to adapt and share recommendation sets within governed bounds while preserving source-set lineage and auditability.
...
This capability sits under `Channel expansion: email and clienteling` and owns one clear responsibility: allow clienteling operators to adapt and share recommendation sets within governed bounds while preserving source-set lineage and auditability.
...
- Prevents downstream consumers from reinterpreting `Clienteling Operator Adaptation And Share` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A stylist edits, curates, or shares a recommendation set during assisted selling.
- A clienteling recommendation needs to be saved or sent to a customer.
- An operator action requires later audit or performance analysis.

## 5. Inputs

- clienteling recommendation payload
- operator role and permission scope
- adaptation policy and allowed operations
- share channel and expiration policy
- governance and suppression context

## 6. Outputs

- adapted recommendation record
- share artifact with scope and expiry
- audit trail of operator interventions
- source-set lineage for analytics

## 7. Workflow / Lifecycle

1. Load the recommendation set and determine which adaptation operations are allowed.
2. Capture operator changes such as removals, substitutions, or notes.
3. Create a share artifact that references the adapted or original set safely.
4. Record the operator action and updated lineage in trace and analytics systems.
5. Enforce expiration and revocation behavior for shared artifacts.
...
- POST /clienteling/shares
- DELETE /clienteling/shares/{shareId}
...
- clienteling.adaptation.saved
- clienteling.share.created
- clienteling.share.revoked
...
1. A request or upstream change triggers `Clienteling Operator Adaptation And Share`.
...
```json
{
  "subFeature": "clienteling-operator-adaptation-and-share",
  "feature": "channel-expansion-email-and-clienteling",
  "input": "clienteling recommendation payload",
  "output": "adapted recommendation record",
...
- Backend services: add or extend a `clienteling-operator-adaptation-and-share` service boundary under the `channel-expansion-email-and-clienteling` domain.
