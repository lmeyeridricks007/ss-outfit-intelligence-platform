...
Render cart recommendation modules that complement existing basket contents without disrupting checkout intent.

## 2. Core Concept

Cart Phase 1 Modules is the capability slice of **Ecommerce Surface Activation** that turns the parent feature intent into a bounded implementation contract with explicit lifecycle, contracts, and operational behavior.

## 3. User Problems Solved

- Downstream teams need cart phase 1 modules behavior that is explicitly defined instead of inferred from the parent feature doc.
- Recommendation-serving paths need implementation-ready contracts for cart phase 1 modules so engineering does not guess at lifecycle or data semantics.
...
- A recommendation refresh is requested in the cart
- Cart module policy changes
...
- Rendered cart module
- Cart-specific telemetry
- Dismissal and refresh state

## 7. Workflow / Lifecycle

1. Build a cart recommendation request from current line items and shopper context.
2. Resolve allowed cart modules and suppressions.
3. Render the chosen modules and track impressions, clicks, saves, and dismissals.

## 8. Business Rules

- Cart modules must prioritize checkout continuity over discovery breadth.
- Cart recommendations cannot contradict current line-item compatibility or eligibility.
- Dismissed modules require explicit refresh semantics.

## 9. Configuration Model

- Cart Phase 1 Modules policy version, rollout state, and owner metadata.
...
- `cart_phase1_modules` primary record storing the current cart phase 1 modules state.
- `cart_phase1_modules_history` append-only history for lifecycle and trace reconstruction.
- `cart_phase1_modules_projection` read model optimized for downstream serving or operator lookup.
...
- POST /internal/ecommerce/cart/modules/resolve
- POST /v1/recommendations

## 12. Events Produced

- CartRecommendationModulesResolved
- EcommerceModuleDismissed
...
- Metrics: request count, success rate, degraded rate, and freshness for cart phase 1 modules.
...
1. API flow: call `POST /internal/ecommerce/cart/modules/resolve` with `Cart line items` and return `Rendered cart module` plus traceable metadata.
2. Event flow: consume `DeliveryResponsePrepared` and emit `CartRecommendationModulesResolved` after business rules and lifecycle checks pass.
3. Operator flow: use `Cart page` to inspect or change cart phase 1 modules behavior, then verify the outcome in the linked trace or health view.

## 23. Implementation Notes

- Backend services involved: `ecommerce-surface-activation-service`, `ecommerce-surface-activation-api`, and `cart-phase1-modules-worker` for async processing.
- Database tables or collections required: `cart_phase1_modules`, `cart_phase1_modules_history`, and `cart_phase1_modules_projection`.
- Jobs or workers required: `cart-phase1-modules-worker`, nightly reconciliation, and replay tooling for operational recovery.
- External APIs or systems required: Storefront cart, Delivery API, Analytics SDK.
- Frontend components required: Cart recommendation rail, Dismiss control.
...
- What refresh cadence is appropriate when cart contents change quickly?
- Should cart dismissals persist across session reloads or only within a visit?
