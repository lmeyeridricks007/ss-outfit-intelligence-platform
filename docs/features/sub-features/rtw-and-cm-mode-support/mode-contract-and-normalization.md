# Sub-feature capability: Mode Contract And Normalization
...
Normalize and preserve explicit mode on every recommendation request and response so RTW and CM semantics do not depend on downstream heuristics.
...
This capability sits under `RTW and CM mode support` and owns one clear responsibility: normalize and preserve explicit mode on every recommendation request and response so rtw and cm semantics do not depend on downstream heuristics.
...
- Prevents downstream consumers from reinterpreting `Mode Contract And Normalization` behavior differently on each surface or channel.
- Keeps cross-cutting uncertainty explicit through open-decision references instead of forcing later stages to guess.

## 4. Trigger Conditions

- A recommendation request enters the system with or without explicit mode data.
- A mixed RTW and CM product context requires deterministic mode selection.
- Delivery or analytics needs mode-aware identifiers and contracts.

## 5. Inputs

- request mode hint
- catalog product mode fields
...
- normalized mode annotation
- mode-aware request context
- contract-safe mode fields in responses
- trace and telemetry mode metadata

## 7. Workflow / Lifecycle

1. Read the incoming request and inspect explicit mode fields or product metadata.
2. Resolve the effective mode using deterministic policy when inputs disagree.
3. Attach the final mode to request, delivery, and trace contexts.
4. Reject or downgrade requests that violate mode-specific constraints.
5. Expose mode lineage for support and analytics.
...
1. A request or upstream change triggers `Mode Contract And Normalization`.
...
```json
{
  "subFeature": "mode-contract-and-normalization",
  "feature": "rtw-and-cm-mode-support",
  "input": "request mode hint",
  "output": "normalized mode annotation",
...
- Backend services: add or extend a `mode-contract-and-normalization` service boundary under the `rtw-and-cm-mode-support` domain.
